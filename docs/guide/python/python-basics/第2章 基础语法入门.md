...
在本章中，我们学习了 Python 的基础语法，包括：

- **变量命名规则**：只能包含字母、数字和下划线，不能以数字开头，区分大小写
- **赋值操作**：使用 `=` 进行赋值，支持多重赋值和动态类型
- **输入输出**：`print()` 用于输出，`input()` 用于获取用户输入（返回字符串）
- **字符串格式化**：推荐使用 f-string（Python 3.6+），也支持 `format()` 和 `%` 格式化

这些基础知识是后续学习的基础，建议多加练习，熟练掌握。